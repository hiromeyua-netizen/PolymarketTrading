...
/**
 * Test script to simulate "follow the previous winner" strategy
 * 
 * Strategy:
 * - Round 1: Check outcome
 * - Round 2: Bet on Round 1's outcome
 * - Round 3: Bet on Round 2's outcome
 * - Continue following the previous round's winner
 * 
 * Goal: Find maximum consecutive failure streak
 */
async function testFollowPreviousWinner() {
  try {
    logger.info('🚀 Starting Follow Previous Winner Strategy Test...');
    ...
    for (const coin of coins) {
      ...
      for (let i = 0; i < slugOutcomes.length; i++) {
        ...
        if (i === 0) {
          ...
          // Bet on the previous outcome
          const bet: Outcome = previousOutcome!;
          totalBets++;
...
if (require.main === module) {
  testFollowPreviousWinner();
}

export default testFollowPreviousWinner;
